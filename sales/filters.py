...
class SaleFilter(django_filters.FilterSet):
    ...
    created_at_date_gte = django_filters.DateFilter(
        field_name="created_at__date", lookup_expr="gte"
    )
    created_at_date_lte = django_filters.DateFilter(
    ...
    # Amount filters with decimal support
    ...
    total_gte = django_filters.NumberFilter(field_name="total", lookup_expr="gte")
    total_lte = django_filters.NumberFilter(field_name="total", lookup_expr="lte")

    subtotal = django_filters.RangeFilter()
    subtotal_gte = django_filters.NumberFilter(field_name="subtotal", lookup_expr="gte")
    subtotal_lte = django_filters.NumberFilter(field_name="subtotal", lookup_expr="lte")

    discount = django_filters.RangeFilter()
    discount_gte = django_filters.NumberFilter(field_name="discount", lookup_expr="gte")
    discount_lte = django_filters.NumberFilter(field_name="discount", lookup_expr="lte")

    amount_due = django_filters.RangeFilter()
    amount_due_gte = django_filters.NumberFilter(
        field_name="amount_due", lookup_expr="gte"
    )
    amount_due_lte = django_filters.NumberFilter(
        field_name="amount_due", lookup_expr="lte"
    )

    # Status filters
    has_amount_due = django_filters.BooleanFilter(method="filter_has_amount_due")
